...
PSTAGE_PKGARCH    = "${BUILD_ARCH}"
#PSTAGE_EXTRAPATH  ?= "/${OELAYOUT_ABI}/"
#PSTAGE_PKGPATH    = "${DISTRO}/${MACHINE}${PSTAGE_EXTRAPATH}"
PSTAGE_PKGPATH    = "abi${OELAYOUT_ABI}/"
PSTAGE_PKGPN      = "${@bb.data.expand('staging-${PN}-${TARGET_ARCH}', d).replace('_', '-')}"
...
#        bb.data.setVar('PSTAGE_PKGPATH', path, d)
    ...
    if pstage_allowed:
        ...
        deps += " stage-manager-native:do_populate_staging"
