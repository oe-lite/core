...
IPKGCONF_TARGET	= "${STAGING_DIR}${layout_sysconfdir}/opkg.conf"
IPKGCONF_SDK	= "${STAGING_DIR}${layout_sysconfdir}/opkg-sdk.conf"
...
do_package_update_index_ipk () {
	...
	for arch in $ipkgarchs; do
		...
		if [ -e ${DEPLOY_DIR_IPK}/${SDK_ARCH}-sdk-$arch/ ] ; then
			touch ${DEPLOY_DIR_IPK}/${SDK_ARCH}-sdk-$arch/Packages
			ipkg-make-index -r ${DEPLOY_DIR_IPK}/${SDK_ARCH}-sdk-$arch/Packages -p ${DEPLOY_DIR_IPK}/${SDK_ARCH}-sdk-$arch/Packages -l ${DEPLOY_DIR_IPK}/${SDK_ARCH}-sdk-$arch/Packages.filelist -m ${DEPLOY_DIR_IPK}/${SDK_ARCH}-sdk-$arch/
		fi
	done
}
...
package_generate_ipkg_conf () {
	mkdir -p ${STAGING_DIR}${layout_sysconfdir}/
	...
	for arch in $ipkgarchs; do
		...
		echo "arch ${SDK_ARCH}-sdk-$arch $priority" >> ${IPKGCONF_SDK}
		priority=$(expr $priority + 5)
		if [ -e ${DEPLOY_DIR_IPK}/$arch/Packages ] ; then
		        echo "src oe-$arch file:${DEPLOY_DIR_IPK}/$arch" >> ${IPKGCONF_TARGET}
		fi
		if [ -e ${DEPLOY_DIR_IPK}/${SDK_ARCH}-sdk-$arch/Packages ] ; then
		        echo "src oe-${SDK_ARCH}-sdk-$arch file:${DEPLOY_DIR_IPK}/${SDK_ARCH}-sdk-$arch" >> ${IPKGCONF_SDK}
		fi
	done
}
...
python do_package_ipk () {
	...
	for pkg in packages.split():
		...
		if not g and bb.data.getVar('ALLOW_EMPTY', localdata) != "1":
			from bb import note
			note("Not creating empty archive for %s-%s" % (pkg, bb.data.expand('${PV}-${PR}', localdata, True)))
